...
describe('git', async function () {
    ...
    describe('diff', async () => {
        ...
        it('diff with ranges', async () => {
            ...
            await fs.createFile(path.join(root, 'folder', 'F3.txt'));
            await fs.writeFile(path.join(root, 'folder', 'F3.txt'), 'F3 content', { encoding: 'utf8' });
            await git.exec(repository, ['add', '.']);
...
async function createGit(): Promise<Git> {
